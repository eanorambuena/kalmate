<template>
  <div class="bg-[#111] border border-[#333] rounded-xl p-3 min-w-[180px] cursor-grab active:cursor-grabbing relative" :style="{ borderLeft: '3px solid ' + color }">
    ...
    <div class="flex items-center gap-2 mb-1">
      ...
      <span class="text-[9px] font-bold text-[#888] uppercase tracking-wider">{{ def.category }}</span>
    </div>

    <p class="text-white text-xs font-medium mb-0.5">{{ label }}</p>
    <p class="text-[#888] text-[8px] mb-2">{{ def.description }}</p>
    ...
    <div v-if="def.type === 'currencyInput'" class="mb-2 space-y-1.5">
      <div class="flex items-center gap-1">
        <span class="text-[7px] text-[#888] w-5">De</span>
        <select
          v-model="data.from"
      ...
      <div class="flex items-center gap-1">
        <span class="text-[7px] text-[#888] w-5">A</span>
        <select
          v-model="data.to"
          class="flex-1 bg-[#222] border border-[#444] rounded px-1.5 py-1 text-white text-[10px] font-mono outline-none focus:border-[#00c853] appearance-none cursor-pointer"
        >
      ...
      <p class="text-[#888] text-[7px] text-center mt-0.5">{{ data.from }}{{ data.to }}=X</p>
    </div>
    ...
    <div v-if="def.type === 'portfolioInput'" class="mb-2">
      <div v-for="(weight, idx) in data.weights" :key="idx" class="flex items-center gap-1 mb-1">
        <span class="text-[7px] text-[#888] w-6">In{{ idx + 1 }}</span>
        <input
          type="number"
          ...
          min="0"
        />
        <span class="text-[7px] text-[#888]">×</span>
      </div>
    </div>
    ...
    <div class="flex items-start justify-between gap-2 mb-1">
      <div v-if="def.type === 'portfolioInput'">
        <div v-for="(weight, idx) in data.weights" :key="'in-' + idx" class="flex items-center gap-1 mb-1">
          ...
          <span class="text-[7px] text-[#888]">In{{ idx + 1 }}</span>
        </div>
      </div>
      <div v-else class="flex flex-col gap-2">
        <div v-for="(inp, i) in def.inputs" :key="'in-' + i" class="flex items-center gap-1 relative">
          ...
          <span class="text-[7px] text-[#888]">{{ inp.label }}</span>
        </div>
      </div>
      <div class="flex flex-col gap-2">
        <div v-for="(out, i) in def.outputs" :key="'out-' + i" class="flex items-center gap-1 relative">
          <span class="text-[7px] text-[#888]">{{ out.label }}</span>
