    ...
    <div class="flex items-start justify-between gap-2 mb-1">
      <div v-if="def.type === 'portfolioInput'">
        <div v-for="(weight, idx) in data.weights" :key="'in-' + idx" class="flex items-center gap-1 mb-1">
          <Handle type="target" :position="Position.Left" :id="`in${idx}`" class="w-2 h-2 !bg-[#2979ff] !border-0" :style="{ left: '-12px', top: `${20 + idx * 24}px` }" />
          <span class="text-[9px] text-[#bbb]">In{{ idx + 1 }}</span>
        </div>
      </div>
      <div v-else class="flex flex-col gap-2">
        <div v-for="(inp, i) in def.inputs" :key="'in-' + i" class="flex items-center gap-1">
          <Handle type="target" :position="Position.Left" :id="inp.id" class="w-2 h-2 !bg-[#2979ff] !border-0" :style="{ left: '-12px', top: `${20 + i * 24}px` }" />
      ...
      <div class="flex flex-col gap-2">
        <div v-for="(out, i) in def.outputs" :key="'out-' + i" class="flex items-center gap-1">
          ...
          <Handle type="source" :position="Position.Right" :id="out.id" class="w-2 h-2 !bg-[#00c853] !border-0" :style="{ right: '-12px', top: `${20 + i * 24}px` }" />
        </div>
      </div>
    </div>
  </div>
