  ...
  <div v-if="open" class="fixed z-30 pointer-events-none" :style="panelPos">
    <div class="w-96 bg-[#111] border border-[#333] rounded-2xl shadow-2xl pointer-events-auto flex flex-col overflow-hidden transition-shadow duration-200 hover:shadow-[0_0_30px_rgba(0,200,83,0.1)]"
      ...
      <div
        ...
        @mousedown="startDrag"
      >
        <span class="text-xs font-bold text-[#00c853] uppercase tracking-wider">{{ $t('pipeline.ai.title') }}</span>
        <button class="text-[#888] hover:text-white text-lg leading-none px-1.5 py-0.5 rounded-lg hover:bg-[#222] transition-colors" @click="close" :title="$t('pipeline.ai.close')">✕</button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-0" ref="messagesRef">
        ...
        <div v-if="generating" class="flex justify-start">
          <div class="bg-[#222] rounded-2xl px-3.5 py-2 text-[#666] text-xs animate-pulse">{{ $t('pipeline.ai.generating') }}</div>
        </div>
      </div>
      ...
      <div v-if="result && !generating" class="px-4 py-1.5 bg-[#00c853]/10 border-t border-[#00c853]/20 text-[#00c853] text-[10px]">
        {{ $t('pipeline.ai.resultSummary', { nodes: result.nodes.length, edges: result.edges.length }) }}
      </div>
      ...
      <div class="p-3 border-t border-[#222]">
        <div class="flex gap-2">
          <input
            v-model="query"
            type="text"
            :placeholder="$t('pipeline.ai.placeholderShort')"
...
async function doGenerate() {
  ...
  if ('error' in plan) {
    ...
    result.value = plan
    const summary = `${plan.nodes.length} nodes, ${plan.edges.length} connections`
